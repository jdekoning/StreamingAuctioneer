...
class AuctionFetcherSpec extends FlatSpec with Matchers with EasyMockSugar with ScalaFutures {
  ...
  whenExecuting(mockedClient) {
    val auctionFetcher = new AuctionFetcher(lastAuctionUpdate, mockedClient)

    it should "auctionFetcher should return correctly return the class in happy flow" in {
      whenReady(auctionFetcher.getNewAuctionData("some_url")) { result =>
        result should be(auctionDataClass.auctions)
      }
    }

    //TODO add unhappy flows
  }
}
